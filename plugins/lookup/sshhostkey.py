...
DOCUMENTATION = """
    lookup: netson.ahvl.sshhostkey
    version_added: "1.0"
    author:
      - Rinck H. Sonnenberg <r.sonnenberg@netson.nl>
    short_description: retrieve or generate an SSH hostkey, stored in hashicorp vault

    description:
      - Generates an SSH hostkey and allows to retrieve it in various formats.
      - If the sshhostkey existed previously, it will retrieve its value.
...
class LookupModule(AhvlLookup):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        self.setopts("OptionsLookupSSHHostKey")

        # return value
        return self.return_secret('sshhostkey', self.opts)
