...
DOCUMENTATION = """
    ...
    short_description: retrieve or generate an SSH key, stored in hashicorp vault

    description:
      - Generates an SSH key and allows to retrieve it in various formats.
      - If the sshkey existed previously, it will retrieve its value.
...
- hosts: localhost
  ...
  - name: 'hvl_password : test lookup'
    debug:
      msg: "{{ lookup('hvl_password', path='mysql', key='myusr1', ret=item) }}"
    loop: "{{ password_outs }}"
"""
...
class LookupModule(AhvlLookup):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        self.setopts("OptionsLookupCredential")

        # return value
        return self.return_secret('credential', self.opts)
