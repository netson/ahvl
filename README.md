...
| ahvl_token                   | no       | string     |                                   | token from ~/.vault-token which is generated by `vault login`                | vault token; required if auth_method is token; it is strongly recommended to only set the token using the environment variable AHVL_CONNECTION_AHVL_TOKEN! |
